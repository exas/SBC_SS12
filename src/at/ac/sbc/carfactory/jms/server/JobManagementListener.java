...
public class JobManagementListener implements MessageListener,
	...
	public void setup() {
		try {
			...
			logger.error("JobManagementListener<" + this.toString()
	...
	public void onMessage(Message inMessage) {
		...
		if (session == null) {
			logger.error("JobManagementListener<"
					+ this.toString()
			...
			if (inMessage instanceof ObjectMessage) {
				...
				if (inObjectMessage.getObject() instanceof CarPartDTO) {
					...
					if (inMessage.getStringProperty("type").equals(
							"updateCarBodyByPainter")) {
						// Painter sent this message to the Queue
						logger.debug("<"
								+ this.hashCode()
								+ ">: Received Msg from Painter (painted Body).");

						if (carPartDTO.getId() != null) {

							CarBody carBody = CarPartDaoSimpleImpl
									.getInstance().getCarBodyById(
											carPartDTO.getId());
							carBody.setId(new Long(carPartDTO.getId()));
							carBody.setPainterWorkerId(carPartDTO
									.getPainterId());
							carBody.setColor(carPartDTO.getBodyColor());

							logger.debug("CarBody:<" + carBody.getId()
									+ ">, Producer:<" + carBody.getProducerId()
									+ ">");
							logger.debug("PainterWorkerId:<"
									+ carBody.getPainterWorkerId() + ">");
							logger.debug("Color:<" + carBody.getColor() + ">");

							// delete from used part Carts and add to
							// FreeCarBody List
							CarPartDaoSimpleImpl.getInstance()
									.deleteCarBodyById(carBody.getId());

							// save into free CarBodyList
							CarPartDaoSimpleImpl.getInstance().saveFreeCarBody(
									carBody);

							// Check if new AssembleJob or BodyPaintJob is
							// available and send JobMsg
							checkForNewJob(outObjectMessage,
									producerAssemblingJob, producerPainterJob);
						}
						...
						if (carPartDTO.getId() != null) {
							...
							case CAR_BODY:
								...
								carBody = new CarBody(carPartDTO.getId(),
										carPartDTO.getProducerId());
								CarPartDaoSimpleImpl.getInstance()
							...
							case CAR_MOTOR:
								...
								carMotor = new CarMotor(carPartDTO.getId(),
										carPartDTO.getProducerId());
								CarPartDaoSimpleImpl.getInstance()
							...
							case CAR_TIRE:
								...
								carTire = new CarTire(carPartDTO.getId(),
										carPartDTO.getProducerId());
								CarPartDaoSimpleImpl.getInstance()
	...
	private void checkForNewJob(ObjectMessage outObjectMessage,
			MessageProducer producerAssemblingJob,
		...
		} else if (CarPartDaoSimpleImpl.getInstance()
				.checkIfSingleBodyPainterJob()) {


			CarBody carBody = CarPartDaoSimpleImpl.getInstance().getSingleBodyPainterJob();

			if(carBody != null) {
				// PainterJob for single Body is available

				logger.debug("<" + this.hashCode()
						+ ">: PainterJob is possible (CarBody<" + carBody.getId()
						+ "> reserved).");

				// save back to in-memory-DB but not in free LIST, into used CarPart
				// List
				CarPartDaoSimpleImpl.getInstance().saveCarBody(carBody);
				...
				logger.debug("JobManagementListener<" + this.toString()
						+ ">: PainterJob with CarBody<" + carBodyDTO.getId()
